...
interface IncomeWithVehicle extends IncomeEntry {
  vehicleModel: string;
  vehiclePlate: string;
}
...
export class IncomeHistoryPage implements OnInit {
  incomes: IncomeWithVehicle[] = [];
  filteredIncomes: IncomeWithVehicle[] = [];
  vehicles: Vehicle[] = [];
  ...
  ngOnInit() {
    ...
    this.vehicleService.getVehicles().subscribe(vehicles => {
      ...
      this.incomes = this.flattenIncomes(vehicles);
      this.aplicarFiltros();
    });
  }
  ...
  get totalReceitas(): number {
    return this.filteredIncomes.reduce((total, e) => total + (e.amount || 0), 0);
  }

  flattenIncomes(vehicles: Vehicle[]): IncomeWithVehicle[] {
    const all: IncomeWithVehicle[] = [];
  ...
  private aplicarFiltros() {
    ...
    this.filteredIncomes = lista.sort((a, b) => {
      const da = a.date ? new Date(a.date).getTime() : 0;
      const db = b.date ? new Date(b.date).getTime() : 0;
      return db - da;
    });
  ...
  editarReceita(item: IncomeWithVehicle) {
    alert('Edição de receita em breve.');
  }

  async removerReceita(item: IncomeWithVehicle) {
    if (!item.id) {
      return;
    }
    ...
    const alerta = await this.alertCtrl.create({
      ...
      buttons: [
        ...
        },
        {
          ...
          role: 'destructive',
          handler: () => {
            this.vehicleService.removerReceita(item.vehiclePlate, item.id!);
            this.vehicleService.getVehicles().subscribe(vehicles => {
              this.incomes = this.flattenIncomes(vehicles);
              this.aplicarFiltros();
            });
